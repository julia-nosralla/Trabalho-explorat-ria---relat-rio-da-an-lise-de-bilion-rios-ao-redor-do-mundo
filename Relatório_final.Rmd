...
```{r fig-sub, echo=FALSE, error=FALSE, message=FALSE, fig.cap='Boxplot: Sexo vs. patrimônio total em bilhões de dólares',out.width='.99\\linewidth',fig.height=4, fig.asp=NA, fig.ncol=1, fig.nrow=2}
Banco_de_dados$gender <- as.factor(Banco_de_dados$gender)
levels(Banco_de_dados$gender)<- c("Feminino", "Masculino")

Objeto_analise <- ggplot(Banco_de_dados, aes(x = genero, y = patrimonio_bilhoes, color = gender)) + geom_boxplot() + scale_y_continuous(trans = "log10") + labs(x = "Sexo", y = "Patrimônios em bilhões", color = "Sexo")
Objeto_analise
```
